/**
 * Test version of export_layer_scores.mjs
 * Runs only first 5 snapshots with minimal rate limiting for fast feedback
 */
...
const OUTPUT_CSV = path.join(__dirname, 'layer_scores_history_TEST.csv');
const ERROR_LOG = path.join(__dirname, 'export_layer_scores_test_errors.log');
...
// First 5 snapshots only (for testing)
const SNAPSHOT_DATES = [
  '2014-01-01', '2014-02-05', '2014-03-05', '2014-04-02', '2014-05-07',
];

const MIN_CALL_GAP_MS = 100; // Fast for unlimited rate limit
...
async function main() {
  log('═'.repeat(70));
  log('TEST: FIRST 5 SNAPSHOTS ONLY');
  log('═'.repeat(70));
  log(`Total snapshots: ${SNAPSHOT_DATES.length}`);
  log(`API rate: minimal (100ms) for unlimited key`);
  ...
  for (let i = startIdx; i < SNAPSHOT_DATES.length; i++) {
    ...
    try {
      log(`Processing snapshot ${i + 1}/${SNAPSHOT_DATES.length}: ${date}`);
      ...
      log(`  ✓ ${date}: L1=${scores.L1} L2=${scores.L2} L3=${scores.L3} L4=${scores.L4} L5=${scores.L5} Master=${scores.master}`);
    } catch (e) {
  ...
  log('');
  log('CSV Output:');
  log(fs.readFileSync(OUTPUT_CSV, 'utf-8'));
}
