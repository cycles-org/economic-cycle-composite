// scratch/market_phase_coherence.mjs
//
// CyclesTV episode analysis: find uncorrelated pairs across major asset
...
//
// Pipeline:
//   1. Fetch ~850 weekly bars per ticker (SearchSymbols → Ensure → GetBars)
//   2. Pearson correlation on weekly returns; identify |r| < 0.3 pairs
//   3. CycleScanner (dtype=4 Kalman) on each ticker in any uncorrelated pair
...
//
// Output: scratch/market_phase_coherence.md  +  scratch/market_phase_coherence.json
//
// Usage:
//   CYCLE_TOOLS_API_KEY=xxx node scratch/market_phase_coherence.mjs
...
const REPORT_PATH = new URL('./market_phase_coherence.md',   import.meta.url);
const JSON_PATH   = new URL('./market_phase_coherence.json', import.meta.url);

// User-supplied Yahoo-style symbols → cycle.tools tickerIds.
// Convention: <SYMBOL>-W:YFI (Yahoo Finance Index data feed, weekly bars).
// SearchSymbols returns nothing for these on this account, so we skip it
// and hard-code the resolution. Verified working via direct GetDatasetSeries.
const SYMBOLS = [
  { user: '^GSPC', desc: 'S&P 500',           tickerId: '^GSPC-W:YFI' },
  { user: 'GLD',   desc: 'Gold ETF',          tickerId: 'GLD-W:YFI' },
  { user: 'TLT',   desc: 'Long bonds ETF',    tickerId: 'TLT-W:YFI' },
  { user: 'DBC',   desc: 'Broad commodities', tickerId: 'DBC-W:YFI' },
  { user: 'EEM',   desc: 'Emerging markets',  tickerId: 'EEM-W:YFI' },
  { user: 'UUP',   desc: 'US Dollar',         tickerId: 'UUP-W:YFI' },
  { user: 'VNQ',   desc: 'Real estate',       tickerId: 'VNQ-W:YFI' },
  { user: 'WEAT',  desc: 'Wheat ETF',         tickerId: 'WEAT-W:YFI' },
];

const TARGET_BARS               = 850;
const MIN_BARS                  = 200;
...
async function searchSymbols(query) {
  const url = `${BASE_URL}/api/data/SearchSymbols?api_key=${API_KEY}&search=${encodeURIComponent(query)}`;
...
(async () => {
  console.log('Market phase-coherence analysis: 8-ticker correlation + cycle + PCM');
  ...
  // ── Step 2: Correlation matrix on weekly returns ──
  console.log('\n## Step 2: Correlation matrix on weekly returns\n');
  ...
  for (const t of validTickers) {
    ...
    try {
      ...
      console.log(`  C${top.cycleLength}w  strength=${top.strength.toFixed(1)}  bartels=${top.bartelsValue.toFixed(1)}  stab=${top.stabilityScore.toFixed(2)}  status=${top.avgPhaseStatus ?? '?'}`);
    } catch (e) {
  ...
  for (const p of pairCycleTable) {
    const aStr = p.periodA != null ? `C${p.periodA}w` : '—';
    const bStr = p.periodB != null ? `C${p.periodB}w` : '—';
  ...
  for (const p of sharedPeriodPairs) {
    ...
    const lowPeriod  = meanPeriod * PCM_BAND_LOW_FRACTION;   // weeks
    const highPeriod = meanPeriod * PCM_BAND_HIGH_FRACTION;  // weeks
    const lowFreq    = 1 / highPeriod;  // cycles/sample (weekly)
    ...
    console.log(`PCM: ${p.a} ⇄ ${p.b}  (mean period ${meanPeriod.toFixed(0)}w, band [${lowPeriod.toFixed(0)}, ${highPeriod.toFixed(0)}]w, trim ${edgeTrim}w)`);
    ...
    const lagWeeks = (meanLag / (2 * Math.PI)) * meanPeriod;
    ...
    console.log(`  Real:    Std=${stdL.toFixed(3)}  circStd=${stdC.toFixed(3)}  meanLag=${meanLag.toFixed(3)} rad ≈ ${lagWeeks.toFixed(1)}w`);
  ...
  for (const r of pcmResults) {
    if (r.error) {
      console.log(`| ${r.a} ⇄ ${r.b} | ${r.r.toFixed(3)} | C${r.meanPeriod.toFixed(0)}w | — | — | ERROR: ${r.error} |`);
    } else {
      console.log(`| ${r.a} ⇄ ${r.b} | ${r.r.toFixed(3)} | C${r.meanPeriod.toFixed(0)}w | ${r.stdCircular.toFixed(3)} | ${r.pValue.toFixed(4)} | ${r.verdict} |`);
    }
  }
  ...
  if (significantPairs.length > 0) {
    ...
    for (const r of significantPairs) {
      ...
      const lagWeeks = Math.abs(r.meanLagWeeks);
      console.log(`${r.a} ⇄ ${r.b}  (${r.verdict}, p=${r.pValue.toFixed(4)})`);
      console.log(`  ${r.a}: C${cA.cycleLength}w, status=${cA.avgPhaseStatus ?? '?'}, avgPhaseScore=${cA.avgPhaseScore ?? '?'}`);
      console.log(`  ${r.b}: C${cB.cycleLength}w, status=${cB.avgPhaseStatus ?? '?'}, avgPhaseScore=${cB.avgPhaseScore ?? '?'}`);
      console.log(`  Mean Δφ = ${r.meanLagRad.toFixed(3)} rad → ${leader} leads ${lagger} by ~${lagWeeks.toFixed(1)} weeks`);
    }
  } else {
  ...
  // ── Markdown report ──
  const md = [];
  md.push('# Market Phase Coherence — Cross-Asset PCM Analysis');
  md.push('');
  md.push(`Run date: ${new Date().toISOString()}`);
  md.push('Branch:   `richard/forcing-analysis`');
  md.push('Source:   `scratch/market_phase_coherence.mjs`');
  md.push('');
  md.push('## Method');
  md.push('');
  md.push(`1. Fetched ~${TARGET_BARS} weekly bars per ticker via SearchSymbols → EnsureCompleteDataset → GetDatasetSeries.`);
  md.push(`2. Pearson correlation on weekly returns; pairs with |r| < ${UNCORRELATED_THRESHOLD} flagged as uncorrelated.`);
  md.push('3. CycleScanner (dtype=4 Kalman, bartelsLimit=49, dominantPeakFinder, useStability) on each ticker in any uncorrelated pair.');
  ...
  md.push('## Correlation matrix (weekly returns)');
  md.push('');
  ...
  md.push('## Cycle detection on uncorrelated tickers (dtype=4 Kalman)');
  md.push('');
  ...
  for (const t of validTickers) {
    ...
    if (!c || c.error) {
      ...
      md.push(`| ${t.user} | C${c.cycleLength}w | ${fmt(c.strength, 1)} | ${fmt(c.bartels, 1)} | ${fmt(c.stability, 2)} | ${c.avgPhaseStatus ?? '?'} |`);
    }
  }
  md.push('');
  ...
  if (pcmResults.length === 0) {
    ...
    md.push('| Pair | Corr | Mean period | Band (w) | n trimmed | Std linear | Std circular | p-value | Verdict |');
    md.push('|------|-----:|------------:|---------:|----------:|-----------:|-------------:|--------:|---------|');
    for (const r of pcmResults) {
      if (r.error) {
        md.push(`| ${r.a} ⇄ ${r.b} | ${r.r.toFixed(3)} | C${r.meanPeriod.toFixed(0)}w | — | — | — | — | — | ERROR: ${r.error} |`);
      } else {
        md.push(`| ${r.a} ⇄ ${r.b} | ${r.r.toFixed(3)} | C${r.meanPeriod.toFixed(0)}w | [${r.band.low.toFixed(0)}, ${r.band.high.toFixed(0)}] | ${r.n} | ${fmt(r.stdLinear)} | ${fmt(r.stdCircular)} | ${fmt(r.pValue, 4)} | **${r.verdict}** |`);
      }
    }
  }
  md.push('');
  ...
  if (significantPairs.length > 0) {
    ...
    for (const r of significantPairs) {
      ...
      const lagger = r.meanLagRad > 0 ? r.b : r.a;
      const lagWeeks = Math.abs(r.meanLagWeeks);
      md.push(`| ${r.a} ⇄ ${r.b} | ${r.verdict} | ${cA.avgPhaseStatus ?? '?'} | ${cB.avgPhaseStatus ?? '?'} | ${fmt(r.meanLagRad)} | ${leader} leads ${lagger} by ~${lagWeeks.toFixed(1)}w |`);
    }
    md.push('');
  }
  ...
  md.push('- **Surrogate**: phase-shuffle preserves amplitude spectrum, randomizes phase, maintains real-conjugate symmetry. Only ticker A is shuffled; ticker B holds fixed.');
